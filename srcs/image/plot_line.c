...
/*   Updated: 2021/11/23 05:57:49 by sotherys         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_plot_line_low(t_image *image, t_pixel *p0, t_pixel *p1)
{
	...
	while (p.x <= p1->x)
	{
		p.cd = ft_color_lerp(p0->cd, p1->cd, (double)(p.x - p0->x) / d.x);
		ft_image_pixel_put(image, &p);
		if (diff > 0)
		{
			p.y += yi;
			diff -= d.x + d.x;
...
static void	ft_plot_line_high(t_image *image, t_pixel *p0, t_pixel *p1)
{
	...
	while (p.y <= p1->y)
	{
		p.cd = ft_color_lerp(p0->cd, p1->cd, (double)(p.y - p0->y) / d.y);
		ft_image_pixel_put(image, &p);
		if (diff > 0)
		{
			p.x += xi;
